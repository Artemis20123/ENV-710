...
第一个最后只留x2 看每一个model LRT和initial model比较，不significant说明可以减少，significant说明得保留原model，然后看AIC，只留x2好

assumption： plots，

dispersiontest(nut1)
overdispersion （chisq），
...
第二题，最开始是三个参数相乘，最后简化为两项相加。binomial

original diagnosis plot不好，需要画DHARMa plot 参考上课ppt

然后测试其它参数（pchisq，persedoR2），加上vif 测共线性

第三题，binomial 要包含random effect
res5 \<- simulateResiduals(fittedModel = o5)
\> plot(res5)
...
require(performance)
check_overdispersion(o3) high dispersion \>2

so, correct overdispersion by adding an observation-level random effect, but residuals don\'t look good
plot(o3)
